...
test("git compare resolve-conflicts uses AI output to rewrite conflict files", { concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(detailPayload.log.request.includes("You can use `npx -y worktreeman api`"), true);
    assert.equal(detailPayload.log.request.includes("`npx -y worktreeman api dev logs read --command <name> [--source stdout|stderr|all]`"), true);
    assert.equal(detailPayload.log.request.includes("`npx -y worktreeman api documents read <document-id>`"), true);
