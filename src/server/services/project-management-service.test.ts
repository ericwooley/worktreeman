...
test("pull-request documents persist metadata and preserve it across updates", async () => {
  ...
  try {
    const created = await createProjectManagementDocument(repoRoot, {
      title: "Pull request rollout",
      summary: "Track review context for the new PR workspace.",
      markdown: "# Pull request rollout\n\n- Add review UI\n",
      kind: "pull-request",
      pullRequest: {
        baseBranch: "main",
        compareBranch: "feature/pull-request-rollout",
        state: "open",
        draft: true,
      },
    ...
    assert.equal(created.document.kind, "pull-request");
    assert.deepEqual(created.document.pullRequest, {
      baseBranch: "main",
      compareBranch: "feature/pull-request-rollout",
      state: "open",
      draft: true,
    ...
    const createdSummary = list.documents.find((entry) => entry.id === created.document.id);
    assert.ok(createdSummary);
    assert.equal(createdSummary.kind, "pull-request");
    assert.deepEqual(createdSummary.pullRequest, {
      baseBranch: "main",
      compareBranch: "feature/pull-request-rollout",
      state: "open",
      draft: true,
    ...
    assert.equal(updated.document.kind, "pull-request");
    assert.deepEqual(updated.document.pullRequest, {
      baseBranch: "main",
      compareBranch: "feature/pull-request-rollout",
      state: "merged",
      draft: false,
    ...
    const commented = await addProjectManagementComment(repoRoot, created.document.id, {
      body: "Merged after final review.",
    });
    assert.equal(commented.document.kind, "pull-request");
    assert.deepEqual(commented.document.pullRequest, {
      baseBranch: "main",
      compareBranch: "feature/pull-request-rollout",
      state: "merged",
      draft: false,
    ...
    const statusUpdated = await updateProjectManagementStatus(repoRoot, created.document.id, "done");
    assert.equal(statusUpdated.document.kind, "pull-request");
    assert.deepEqual(statusUpdated.document.pullRequest, {
      baseBranch: "main",
      compareBranch: "feature/pull-request-rollout",
      state: "merged",
      draft: false,
    ...
    const fetched = await getProjectManagementDocument(repoRoot, created.document.id);
    assert.equal(fetched.document.kind, "pull-request");
    assert.deepEqual(fetched.document.pullRequest, {
      baseBranch: "main",
      compareBranch: "feature/pull-request-rollout",
      state: "merged",
      draft: false,
